...
library(readxl)     # For reading xls, xlsx files.
...
CleanTrackingData = function (CSVfile, CountryID ) {
  ...
  #if (endsWith(CSVfile, 'csv')){  Data = read.csv(CSVfile) }
  #if (endsWith(CSVfile, 'xlsx')){  Data = read.csv(CSVfile) }
  Data = loadData(CSVfile, file_ext(CSVfile))
  RawData = Data
  RawN = nrow(Data)
  ...
  if ('wv_ freemarket_lim' %in% colnames(Data)){ Data %<>% rename('wv_.freemarket_lim' = 'wv_ freemarket_lim')}
...
RankOrder = function(Data, ColStartsWith, Qids, OrderBy, Title, TxtSize, TitleSize, LabSize ){
  ...
  p = ggplot(data = M, mapping = aes(x = variable, y = Names)) +
    ...
    scale_y_discrete(expand = c(0,0) ) + #scale_x_discrete(expand = c(0,0) ) + 
    guides(fill = guide_colourbar(ticks = FALSE, title="CDF%", size = 10)) + 
...
loadData = function(fname, ext){
  pathname = fname
  fname = basename(fname)
  # Clean filename and extention strings
  #   so there are no leading/trailing 
  #   '.'s or extensions.
  if ( startsWith(ext, '.')){
    ext = gsub('\\.','',ext)
  }
  ...
  fname = file.path(dirname(pathname), fname)
  
  # Load in data from different extensions
  if (ext == 'dat'){
...
HDIsummary = function(MCMCobj, Lvl1, Lvl2){
  # Get Mean Cutpoints
  S = summary(MCMCobj)$statistics
  Cuts = c(0, S[grep(x = rownames(S), pattern = 'gamma'),1])
  
  # Get Posterior estimates for each factor level
  meanposts <- tidy_draws(MCMCobj) %>%
    dplyr::select(-starts_with('gamma')) 
  
  # Make storage tbl
  meanposts2 = meanposts
  
  n = meanposts %>% gather_variables() %>% select(.variable) %>% unique %>% t
  
  for (ii in seq(Lvl1)){
    idx1 = grep(Lvl1[ii], n[1,])
    for (kk in seq(Lvl2)){
      idx2 = grep(Lvl2[kk], n[1,])
      l1 = length(idx1); l2 = length(idx2)
      if (l1 > 0 & l2 > 0){
        meanposts2[n[idx1[idx1 %in% idx2]]] = rowSums( meanposts[ n[c(min(idx1), min(idx2), idx1[idx1 %in% idx2])] ], na.rm=T )
      }
    }
  }
  meanposts2$`(Intercept)` = 0
  meanposts2[,colnames(meanposts2) %in% n] = meanposts2[,colnames(meanposts2) %in% n] + meanposts$`(Intercept)`
  
  # Sum Posterior estimates for plotting
  meanposts = meanposts2 %>% gather_variables() %>% mode_hdi()
  
  n = meanposts$.variable
  meanposts$Factor1 = NA
  meanposts$Factor2 = NA
  
  for (ii in seq(Lvl1)){
    meanposts$Factor1[grep(Lvl1[ii], n)] = Lvl1[ii]
  }
  meanposts$Factor1[is.na(meanposts$Factor1)] = Lvl1[1]
  
  for (kk in seq(Lvl2)){
    meanposts$Factor2[grep(Lvl2[kk], n)] = Lvl2[kk]
  }
  meanposts$Factor2[is.na(meanposts$Factor2)] = Lvl2[1]
  
  meanposts$Factor1 = factor(meanposts$Factor1, levels = Lvl1)
  meanposts$Factor2 = factor(meanposts$Factor2, levels = Lvl2)
  
  return(list(Means = meanposts, Cutpoints = Cuts))
}

Siglines = function(HDIs, pos, posoffset, yoffset){
  if (missing(pos)){ pos = c(-2, -1, 1, 2) }
  if (missing(posoffset)){ posoffset = .14 }
  if (missing(yoffset)){ yoffset = .05 }
  ...
  for (Qs in seq(F2)){
    Check = c()
    maxY = M %>% filter(Factor2 == F2[Qs]) %>% select(.upper) %>% max + yoffset
    for (W2 in seq(F1)){
      for(W1 in seq(F1)){
        sqr = W1^2 + W2^2
        if (W1 != W2 & !(sqr %in% Check)){
          Check = c(Check, sqr)
          D1 = M %>% filter(Factor1 == F1[W1] & Factor2 == F2[Qs]) %>% select(.lower, .upper)
          D2 = M %>% filter(Factor1 == F1[W2] & Factor2 == F2[Qs]) %>% select(.lower, .upper)
          below = D1[1] < D2[1] & D1[2] < D2[1] # min & max below bounds
          above = D1[1] > D2[2] & D1[2] > D2[2] # min & max above bounds
          if (below == T | above == T){
            maxY = maxY + yoffset
            y = c(y, maxY)
            xi = c(xi, Qs + pos[W1]*posoffset)
            xe = c(xe, Qs + pos[W2]*posoffset)
          }
        }
      }
    }
  }
  return(list(y=y, xi=xi, xe=xe))
}
